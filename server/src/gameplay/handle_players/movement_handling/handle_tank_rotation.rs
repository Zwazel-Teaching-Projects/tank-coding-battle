...
use shared::{
    ...
    networking::messages::{
        message_container::RotateTankBodyCommandTrigger,
        message_data::tank_messages::RotationDirection,
    },
};

pub fn handle_tank_body_rotation(
    trigger: Trigger<RotateTankBodyCommandTrigger>,
    mut clients: Query<(&mut TankTransform, &TankType)>,
    tank_config: TankConfigSystemParam,
) {
    let client_entity = trigger.entity();
    let (mut tank_transform, tank_type) = clients
    ...
    let rotation_direction = match trigger.direction {
